...
const HomePage = () => {
  ...
  const tags = [
    {
      tag: 1,
      header: "Equipment for outdoor boulderingr",
      button: "Rent Climbing Gear",
      image:
        "https://s3.amazonaws.com/www.explorersweb.com/wp-content/uploads/2022/03/03122909/best-waterproof-tents-header-16.jpg",
    },
    {
      tag: 2,
      header: "Surif in Pacifica or Ocean Beach",
      button: "Rent Surfing Gear",
      image:
        "https://images.unsplash.com/photo-1673214846075-d6c357890103?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=687&q=80",
    },
    {
      tag: 3,
      header: "Bouldering Gear",
      button: "Rent Camping Gear",
      image:
        "https://images.unsplash.com/photo-1671726805768-575f88de945a?ixlib=rb-4.0.3&ixid=MnwxMjA3fDF8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1170&q=80",
    },
  ];
          ...
          <div className="flex space-x-5">
            <p
              ...
              className={`w-[50px] h-[5px] cursor-pointer ${
                activeTag.tag === 1 ? " bg-black" : "bg-slate-300"
              }`}
            ></p>
            <p
              onClick={() => handleClick(2)}
              className={`w-[50px] h-[5px] cursor-pointer ${
                activeTag.tag === 2 ? " bg-black" : "bg-slate-300"
              }`}
            ></p>
            <p
              onClick={() => handleClick(3)}
              className={`w-[50px] h-[5px] cursor-pointer ${
                activeTag.tag === 3 ? " bg-black" : "bg-slate-300"
              }`}
            ></p>
          </div>
      ...
      <section className=" py-20 h-full w-[85%] mx-auto">
        <ProductGridSection title={"Most popular"} Button={Button}>
          {MostPopular.map((item) => (
            <ProductCard
              className=" xl:border-l mt-4  sm:h-64 sm:w-64 
               md:h-96 md:w-96 lg:h-128 lg:w-128 xl:h-160 
               xl:w-160 flex  flex-col items-center 
               justify-center bg-white hover:shadow-xl"
              text={item.name}
              src={item.img}
            />
          ))}
        ...
        <ProductGridSection title={"Climbing"} Button={Button}>
          {ClimbingProductsLanding.map((item) => (
            <ProductCard
              ...
              src={item.img}
              className=" xl:border-l mt-4  sm:h-64 sm:w-64  
              md:h-96 md:w-96 lg:h-128 lg:w-128 xl:h-160 xl:w-160 
        ...
        bg-[url("https://images.unsplash.com/photo-1522163182402-834f871fd851?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1103&q=80")]  '>
          <div className="w-1/2 ">
            <p className="md:text-6xl font-bold ">
              {" "}
              V18 Rentals: An Environmental and Ethos Based Take on the Gear
              Rental Industry
            </p>
            <div className="mt-10">
      ...
      <section className="md:py-20 h-full md:w-[85%] mx-auto md:flex  grid grid-cols-1 md:px-0 px-10  ">
        <div className="md:w-1/2 h-[400px]  bg-black text-white mr-8 w-full">
          <div className="h-[200px] m-6">
            <header className="h-[50px] text-3xl font-bold">
              Keep in touch
            </header>

            <main className="py-10">
              Be the first to find out about V18 events, new store openings, and
              more.
            </main>
            <Button
              className={"bg-green-500 p-4 my-20 rounded text-white text-xl"}
              value={"Sign me up!"}
              onClick={openModal}
            />
          </div>
        ...
        <div className="md:w-1/2 w-full h-[400px] bg-white mt-10 md:mt-0  text-black shadow-xl border">
          <div className="h-[200px] m-6">
            ...
            <main className="py-10">
              Give a friend $5 in their account, and earn $5 in your account
