import React from 'react'

const Cart = () => {
  return (
    <div className='p-6 md:flex md: justify-around block h-screen '>
<div>
        <p className='text-xl'>Your cart</p>
        <p className='my-10'> Tue January 10 - Tue January 10, 2023</p>
        <p className='my-10 text-4xl text-gray-400 '>Your cart is empty</p>
        </div>

<div className='bg-gray-200 rounded-md h-[150px] md:h-[150px] md:w-1/4 p-2 flex items-center justify-center'>
Notice
V18 Rentals offers bouldering equipment rentals with delivery & pickup service to any address in San Francisco. Orders must be booked by midnight the day before the rental begins.
</div>
    </div>
  )
}

export default Cart